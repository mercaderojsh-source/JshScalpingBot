        ...
        if (
            ...
            for item in ranked:
                ...
                if exit_signal:
                    ...
                    # Fetch updated statistics to display on Telegram
                    account_stats = trader.stats() if not LIVE_TRADING else performance_summary()

                    send_message(
                        f"💰 TRADE CLOSED\n\n"
                        f"Pair : {trade['pair']}\n"
                        f"Exit Reason : {exit_signal}\n"
                        f"PnL : ${trade['pnl']:+.2f}\n"
                        f"Balance : ${trade['balance']:.2f}\n\n"
                        f"📊 STATS\n"
                        f"Trade : {account_stats['trades']}\n"
                        f"Win : {account_stats['wins']}\n"
                        f"Loss : {account_stats['losses']}\n"
                        f"Winrate : {account_stats['win_rate']}%"
                    )
